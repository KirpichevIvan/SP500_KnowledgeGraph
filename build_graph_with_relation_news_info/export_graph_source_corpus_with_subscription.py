...
DEFAULT_OUT_TXT = os.path.abspath(os.path.join(BASE_DIR, "../data/graph_source_corpus_with_subscription.txt"))
...
def format_company_block(row: pd.Series, wiki_summary: str) -> str:
    """Сырой текст с минимальными подписями полей (без лишней служебной инфы)."""
    ordered_fields = [
        "Ticker",
        "Name",
        "Sector",
    ...
    if wiki_summary:
        pieces.append(f"Wikipedia_Summary: {wiki_summary}")
    ...
    with open(output_txt, "w", encoding="utf-8") as f:
        ...
        if include_news_csv and os.path.exists(news_csv_path):
            ...
            for _, nrow in news_df.iterrows():
                labeled_values = []
                for k, v in nrow.to_dict().items():
                    sv = str(v).strip()
                    if sv and sv.lower() != "nan":
                        labeled_values.append(f"{k}: {sv}")
                f.write("\n".join(labeled_values) + "\n\n")
